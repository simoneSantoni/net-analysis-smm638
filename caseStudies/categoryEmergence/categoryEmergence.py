...
Signed-off-by: Matteo Devigili, matteo.devigili.2@city.ac.uk

Date: Thu 24 Nov 2022 15:47:24 GMT

Notes:  This script shows how to:
		- appreciate core-periphery structures via heatmap
...
# %% Core-periphery -- heatmap
#--+ get nodes sorted
n_ = sorted(g_c.degree, key=lambda x: x[1], reverse=True)
n_ = [i[0] for i in n_]
#--+ extract matrix
na = nx.to_numpy_array(g_c, nodelist=n_, dtype=np.float)
#--+ plot data
fig = plt.figure(figsize=(6, 6))
ax = fig.add_subplot(1, 1, 1) 
sns.heatmap(na, center=0, cbar=False, cmap='coolwarm', ax=ax)
plt.axis('off')

# %% Explore communities
# --+ fit
solutions = nx.algorithms.community.girvan_newman(g_c)
# alternative partioning solutions to consider
k = 19
