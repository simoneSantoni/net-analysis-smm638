...
ML_group = ["Prof_A", "PhD_1", "PhD_2"]
Theory_group = ["Prof_B", "PhD_3", "PhD_4"]

# Positions (based on role similarity)
Senior = ["Prof_A", "Prof_B"]  # Both advise
Junior = ["PhD_1", "PhD_2", "PhD_3", "PhD_4"]  # All advised
```
...
G.add_edges_from([(5, 15), (7, 17), (2, 12)])
...
- **Strong**
- **Weak**
- **Density-based operational definitions**
...
Definition of modularity; modularity maximization.
...
communities_fine = louvain(G, resolution=2.0)  # 8 communities

# Default resolution
communities_default = louvain(G, resolution=1.0)  # 4 communities

# Lower resolution → fewer, larger communities  
communities_coarse = louvain(G, resolution=0.5)  # 2 communities
```
...
Intuition, approaches, use cases.

---

## Rendering Roles: Algorithms

Blockmodeling, equivalence types, clustering.

---

## Rendering Positions in Network Data

Triad profiles, multiplexity, centrality patterns.

---

## Rendering Positions: Algorithms

Similarity matrices, clustering, blockmodels.
...
# Nodes with similar tie profiles have high correlation
corr = np.corrcoef(A)
equivalence = corr  # Higher = more equivalent
```
